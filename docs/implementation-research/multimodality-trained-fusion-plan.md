...
| T2 matched-recipe retrain | done - gate FAILED | new config `dpcode/conf/experiment/pam50_wsi_cnv_matched.yaml`; runs `.scratch/results/pam50_wsi_cnv_matched_{gated,coattn}_s1/` (10 folds each; one 3090 each via `CUDA_VISIBLE_DEVICES` 0/1 as two separate `dp-train experiment=pam50_wsi_cnv_matched fusion=<op>` invocations; gated 49 min, coattn 42 min); concat/cross_attention/film_attention NOT run (gate failed); analysis `.scratch/analysis/compare_fusion_ladder/2026-09-04_12-35-46/`; reproduce with `python -m dpcode.cli.analysis compare_fusion_ladder 'analysis.run_pattern="pam50_wsi_cnv_matched_{mode}_s1"' 'analysis.compare_to="pam50_wsi_cnv_{mode}_s1"'`; `tools/compare_fusion_ladder.py` gained `--run-pattern` / `--compare-to` (threaded through `dp-analysis` as `analysis.run_pattern` / `analysis.compare_to`; default output unchanged; a run dir with no `split_*_results.pkl` yet is now skipped instead of crashing) | pooled macro AUROC on 599 cases (N=2000 seed 13) gated 0.8928 [0.872, 0.914], coattn 0.8992 [0.879, 0.919] vs WSI 0.8872 / CNV 0.8721 / mean 0.9259; delta vs mean gated -0.0330 [-0.0509, -0.0172] sig, coattn -0.0267 [-0.0433, -0.0107] sig; delta vs the original rounded-recipe run of the same operator (paired, same resamples) gated -0.0019 [-0.0085, +0.0043] ns, coattn -0.0000 [-0.0003, +0.0002] ns; fold-mean test_auc matched/original gated 0.8946/0.8972, coattn 0.8936/0.8934; phi between the two matched operators 0.637 (unimodal 0.193); gated+coattn ensemble 0.9084, delta vs mean -0.0174 sig. Gate: neither beats -0.014, FAIL. Caveat (verified in code this session): on the multimodal path CLAM routes fusion runs to `train_loop`, not `train_loop_clam` (`project/CLAM/utils/core_utils.py:434` has `and not _is_multimodal(args)`), and `CLAMRNAFusion` is built without `args.inst_loss` and its `forward` ignores `label`/`instance_eval`, so `inst_loss svm`, `bag_weight 0.553` and instance clustering are recorded in `experiment_*.txt` but inert; the only live change from the `_s1` ladder was lr/reg at full precision. T2 therefore refutes only the lr/reg-rounding part of diagnosis 2 and doubles as the first run-to-run variance measurement of the ladder (paired delta ~0). The instance-clustering/SVM part of the recipe stays untested; making it live in fusion is a model/training-loop change in T5's file set (`model_multimodal.py`, `core_utils.py`, `main.py`), not a config change. T3/T4 should proceed on the matched config anyway (it is the baseline's recipe as far as the multimodal path can express it) - or on `pam50_wsi_cnv`, since the two are equivalent. | 2026-09-04 |
